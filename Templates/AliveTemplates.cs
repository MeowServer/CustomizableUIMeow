            ...
            try
            {
                if (PlayerUICommonTools.IsSCP(ev.Player) && ev.Player.Role.Type != RoleTypeId.Scp0492 && ev.Player.Health <= ev.Player.MaxHealth * 0.2)
                {
                    if (lastTimeHurtInformed.ContainsKey(ev.Player))
                    {
                        ...
                        {
                            SCPInformations.Insert(0, new SCPEventHint(SCPEventHint.SCPEventType.SCPLowHP, $"<b><color=#D32F2F>⚠</color></b>{UIPluginConfig.instance.GeneralConfig.RoleName[ev.Player.Role.Type]}的血量较低|血量：{(int)ev.Player.Health}", Round.ElapsedTime));
